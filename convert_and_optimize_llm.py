...
def optimize_model_for_npu(model):
    ...
    # Only reshape if it's an embedding model (has reshape method)
...
def convert_chat_model(model_type: str, precision: str, model_dir: Path, access_token: str | None) -> Path:
    ...
    print(f"Loading and converting {model_name} model...")
    try:
        # load model and convert it to OpenVINO with memory optimizations
        ...
        print("Applying NPU optimizations...")
        optimize_model_for_npu(model)
        
    except (OSError, RuntimeError, MemoryError) as e:
        print(f"Error loading model {model_name}: {e}")
        print("This might be due to insufficient memory. Try using a smaller model like qwen2-1.5B or qwen2-0.5B")
        raise
...
if __name__ == "__main__":
    ...
    parser.add_argument("--chat_model_type", type=str, choices=["qwen2-7B", "qwen2-1.5B", "qwen2-0.5B", "llama3.2-3B", "llama3.2-1B"],
                        default="qwen2-1.5B", help="Chat model to be converted")
